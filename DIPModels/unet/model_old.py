...
def unet_2d_basic(x, filter_size, dropout_rate=0.0, use_batch_norm=True):
    ...
    conv_1 = conv_block_double(x, 1*filter_size, dropout_rate, use_batch_norm, name='conv_1')
    pool_2 = MaxPooling2D(pool_size=(2, 2), name='conv_1_pool')(conv_1)

    conv_2 = conv_block_double(pool_2, 2*filter_size, dropout_rate, use_batch_norm, name='conv_2')
    pool_3 = MaxPooling2D(pool_size=(2, 2), name='conv_2_pool')(conv_2)

    conv_3 = conv_block_double(pool_3, 4*filter_size, dropout_rate, use_batch_norm, name='conv_3')
    pool_4 = MaxPooling2D(pool_size=(2, 2), name='conv_3_pool')(conv_3)

    conv_4 = conv_block_double(pool_4, 8*filter_size, dropout_rate, use_batch_norm, name='conv_4')
    pool_5 = MaxPooling2D(pool_size=(2, 2), name='conv_4_pool')(conv_4)

    conv_5 = conv_block_double(pool_5, 16*filter_size, dropout_rate, use_batch_norm, name='conv_5')
    pool_6 = MaxPooling2D(pool_size=(2, 2), name='conv_5_pool')(conv_5)

    conv_6 = conv_block_double(pool_6, 32*filter_size, dropout_rate, use_batch_norm, name='conv_6')

    up_5 = Conv2D(16*filter_size, 2, activation='relu', padding='same', name='up_5_upconv')(UpSampling2D(size=(2, 2))(conv_6))
    up_5 = Concatenate(axis=bn_axis, name='up_5_concat')([up_5, conv_5])
    up_conv_5 = conv_block_double(up_5, 16*filter_size, dropout_rate, use_batch_norm, name='up_5')

    up_4 = Conv2D(8*filter_size, 2, activation='relu', padding='same', name='up_4_upconv')(UpSampling2D(size=(2, 2))(up_conv_5))
    up_4 = Concatenate(axis=bn_axis, name='up_4_concat')([up_4, conv_4])
    up_conv_4 = conv_block_double(up_4, 8*filter_size, dropout_rate, use_batch_norm, name='up_4')

    up_3 = Conv2D(4*filter_size, 2, activation='relu', padding='same', name='up_3_upconv')(UpSampling2D(size=(2, 2))(up_conv_4))
    up_3 = Concatenate(axis=bn_axis, name='up_3_concat')([up_3, conv_3])
    up_conv_3 = conv_block_double(up_3, 4*filter_size, dropout_rate, use_batch_norm, name='up_3')

    up_2 = Conv2D(2*filter_size, 2, activation='relu', padding='same', name='up_2_upconv')(UpSampling2D(size=(2, 2))(up_conv_3))
    up_2 = Concatenate(axis=bn_axis, name='up_2_concat')([up_2, conv_2])
    up_conv_2 = conv_block_double(up_2, 2*filter_size, dropout_rate, use_batch_norm, name='up_2')

    up_1 = Conv2D(1*filter_size, 2, activation='relu', padding='same', name='up_1_upconv')(UpSampling2D(size=(2, 2))(up_conv_2))
    up_1 = Concatenate(axis=bn_axis, name='up_1_concat')([up_1, conv_1])
    up_conv_1 = conv_block_double(up_1, 1*filter_size, dropout_rate, use_batch_norm, name='up_1')
...
def unet_2d(x, filter_size, layer, dropout_rate=0.0, use_batch_norm=True, use_conv_after_upsampling=True):
    bn_axis = -1 if K.image_data_format() == 'channels_last' else 1
    
    conv_layers = []
    for i in range(layer-1):
        name = 'conv_' + str(i+1)
        conv = conv_block_double(x, (2**i)*filter_size, dropout_rate, use_batch_norm, name=name)
        conv_layers.append(conv)
        x = MaxPooling2D(pool_size=(2, 2), name=name + '_pool')(conv)
    
    x = conv_block_double(x, 2**(layer-1)*filter_size, dropout_rate, use_batch_norm, name='conv_' + str(layer))
    
    for i in range(layer-2, -1, -1):
        name = 'up_' + str(i+1)
        up = UpSampling2D(size=(2, 2))(x)
        if use_conv_after_upsampling:
            up = Conv2D((2**i)*filter_size, 2, activation='relu', padding='same', name=name+'_upconv')(up)
        x = Concatenate(axis=bn_axis, name=name + '_concat')([up, conv_layers[i]])
        x = conv_block_double(x, (2**i)*filter_size, dropout_rate, use_batch_norm, name=name)
...
class UNet(object):
    ...
    def build_model(self, config, model_dir, weights_path, **kwargs):
        ...
        # Image size must be dividable by 2**self.nb_layers
